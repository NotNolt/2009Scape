package jagex;/* jagex.Class1_Sub8 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class1_Sub8 extends Class1
{
    public String aString3593;
    public int anInt3594;
    public static int anInt3595;
    public int anInt3596;
    public static int anInt3597;
    public int anInt3598;
    public int anInt3599;
    public static int anInt3600 = 0;
    public int anInt3601;
    
    public static void method140(byte arg0, int arg1) {
	try {
	    anInt3597++;
	    int i = 101 % ((arg0 - 37) / 47);
	    Class131_Sub41_Sub6 class131_sub41_sub6
		= Class131_Sub13.method1709(8, arg1, (byte) 1);
	    class131_sub41_sub6.method1959(-97);
	} catch (RuntimeException runtimeexception) {
	    throw Class131_Sub2_Sub6.method1495(runtimeexception,
						("vh.A(" + arg0 + ',' + arg1
						 + ')'));
	}
    }
}
